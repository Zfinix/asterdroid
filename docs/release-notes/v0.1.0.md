...
Keys are plain environment variables named in the catalog's `key_env` field, such as `OPENROUTER_API_KEY` for OpenRouter. Enter the key for whichever provider you selected. A local OpenAI-compatible endpoint also works: point `ASTER_BASE_URL` at it.
